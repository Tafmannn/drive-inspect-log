-- Rollback for 20260629100000_fix_damage_items_replay_ordering
-- Restores submit_inspection to the prior definition (20260428081955):
--   - replay returns array_agg(id ORDER BY created_at)
--   - damage insert without submission_index / WITH ORDINALITY
-- The additive damage_items.submission_index column is left in place (harmless;
-- drop manually only if you are certain nothing depends on it).
--
-- NOTE: reverting re-introduces the non-deterministic replay ordering (V1).
