-- =====================================================================
-- Server-side enforcement: one-job-at-a-time + blocked-status checks
-- =====================================================================
-- DEFECT: evaluateExecutableState()'s "blocked" rules (non-actionable job
-- status; driver already has another job in pickup_in_progress/
-- delivery_in_progress) were only enforced client-side (JobDetail hides/
-- disables the action). A direct call to the submit_inspection RPC bypassed
-- both checks entirely.
--
-- FIX: mirror both checks server-side, inside the same row-locked
-- transaction that already reads jobs.status/driver_id FOR UPDATE, before
-- any inspection/damage_item writes occur.
-- =====================================================================
...
-- The 20260629100000 migration added this 5-arg overload (with
-- p_submission_session_id DEFAULT NULL) but never dropped the original
-- 4-arg submit_inspection. Both real call sites (submitQueue.ts, api.ts)
-- always pass all 5 named params, so the 4-arg overload has been dead code
-- since that migration — and it predates (and therefore lacks) the
-- JOB_NOT_ACTIONABLE/ACTIVE_JOB_LOCK checks above, plus its continued
-- existence makes any 4-positional-arg call ambiguous ("function is not
-- unique") instead of cleanly resolving.
