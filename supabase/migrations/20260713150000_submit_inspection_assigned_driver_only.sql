-- =====================================================================
-- WORKFLOW-003: submit_inspection must only accept the assigned driver
-- =====================================================================
-- DEFECT: submit_inspection() reads jobs.driver_id (v_driver_id) only to
--   check whether the CALLING driver has a *different* job already in
--   pickup_in_progress/delivery_in_progress (the ACTIVE_JOB_LOCK check added
--   in 20260713120000). Nothing anywhere in the function verifies that the
--   calling user IS the job's assigned driver. Combined with jobs_update_org
--   / the inspections & damage_items RLS policies being scoped by org only
--   (not by assignee), any active, non-suspended driver in the same org
--   could open /inspection/:jobId/:type for a job assigned to a DIFFERENT
--   driver (or an unassigned job) and submit a full pickup/delivery
--   inspection against it — undermining the entire assignment/eligibility
--   workflow (AssignDriverModal's licence/onboarding compliance checks exist
--   specifically to gate who should be doing this).
--
-- FIX: mirror the existing pattern (server-side check alongside the
--   JOB_NOT_ACTIONABLE / ACTIVE_JOB_LOCK checks already added in
--   20260713120000): reject the call unless the caller is an admin/
--   super-admin, OR the caller's driver_profiles row matches jobs.driver_id.
--   An unassigned job (driver_id IS NULL) is rejected for non-admins too —
--   there is no legitimate assigned driver yet to authorise.
--
-- Everything else in this function is byte-for-byte unchanged from
-- 20260713120000 — only the new authorization check is inserted, immediately
-- after the existing JOB_NOT_FOUND check and before any other logic, so it
-- fails closed as early as possible and before any row is touched.
-- =====================================================================
