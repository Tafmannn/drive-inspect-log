-- =====================================================================
-- WORKFLOW-004: a delivery inspection must not be accepted before pickup
-- =====================================================================
-- DEFECT: submit_inspection() reads jobs.has_pickup_inspection (v_has_pickup)
--   only to decide which terminal status a delivery submission lands on
--   (pod_ready vs delivery_complete) — it never rejects a delivery submission
--   when pickup was never done. The only gate preventing this is client-side:
--   JobDetail's derivePrimaryCta() always routes to "Start Pickup" when
--   !hasPickup, so the "Start Delivery" button is unreachable through the
--   UI — but nothing stops a driver from navigating directly to
--   /inspection/:jobId/delivery (the same bypass vector as WORKFLOW-003) and
--   submitting a full delivery inspection for a job whose status is still
--   e.g. "ready_for_pickup". That status is not in v_non_actionable_statuses,
--   so JOB_NOT_ACTIONABLE does not catch it either. The result: a vehicle
--   logistics job reaches delivery_complete with no collection-time
--   odometer/fuel/damage/signature record at all — a real operational/
--   compliance gap, not just a display quirk (admin review does surface a
--   "No pickup insp." badge, so it is not silently hidden, but the invalid
--   transition itself was never blocked).
--
-- FIX: reject a delivery submission when has_pickup_inspection is false,
-- unless the caller is an admin/super-admin (mirrors the existing
-- admin-bypass convention already used for the NOT_ASSIGNED_DRIVER check in
-- 20260713150000 — an admin may legitimately force a delivery-only
-- completion, e.g. a vehicle already at the yard with no collection leg).
--
-- Everything else in this function is byte-for-byte unchanged from
-- 20260713150000 — only the new check is inserted, immediately after the
-- NOT_ASSIGNED_DRIVER check and before JOB_NOT_ACTIONABLE, so it fails
-- closed as early as possible and before any row is touched.
-- =====================================================================
...
BEGIN
  ...
  -- ── WORKFLOW-003: only the assigned driver (or an admin/super-admin) may
  -- submit an inspection for this job. An unassigned job (v_driver_id IS
  -- NULL) has no legitimate assigned driver yet, so non-admins are rejected.
  IF NOT public.is_admin_or_super_admin() THEN
    IF v_driver_id IS NULL OR NOT EXISTS (
      SELECT 1 FROM public.driver_profiles dp
      WHERE dp.id = v_driver_id AND dp.user_id = auth.uid()
    ) THEN
      RAISE EXCEPTION 'NOT_ASSIGNED_DRIVER'
        USING ERRCODE = '42501',
              HINT = 'Only the assigned driver or an admin can submit this inspection';
    END IF;
  END IF;

  -- ── WORKFLOW-004: a delivery inspection requires pickup to be complete
  -- first, unless the caller is an admin/super-admin forcing a delivery-only
  -- completion.
  IF p_type = 'delivery' AND NOT v_has_pickup AND NOT public.is_admin_or_super_admin() THEN
    RAISE EXCEPTION 'PICKUP_NOT_COMPLETE'
      USING ERRCODE = '23514',
            HINT = 'Complete the pickup inspection before delivery';
  END IF;
