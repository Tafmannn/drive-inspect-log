-- =====================================================================
-- V1 fix: deterministic damage_items ordering for idempotent replay
-- =====================================================================
-- DEFECT: submit_inspection's idempotency short-circuit returned
--   array_agg(id ORDER BY created_at). All damage_items inserted in one
--   submit share created_at (= transaction timestamp), so the tie-broken
--   order is NOT guaranteed. The offline submit queue correlates damage
--   close-up photos to damage_items strictly by ARRAY INDEX, so on a
--   replay (RPC committed but response lost) the order could differ from
--   the original input — linking photos to the WRONG damage item, or
--   failing promotion (→ photos stranded in "staged" → TTL purge → loss).
--
-- FIX:
--   1. Add a stable per-submission ordinal to damage_items.
--   2. Populate it from the input array position on insert (WITH ORDINALITY).
--   3. Return damageItemIds ordered by that ordinal on BOTH the fresh and
--      the replay path, so the client's positional mapping is always correct.
-- Additive + backwards compatible (legacy rows have NULL ordinal and fall
-- back to created_at, id).
-- =====================================================================

ALTER TABLE public.damage_items
  ADD COLUMN IF NOT EXISTS submission_index integer;
...
BEGIN
  ...
  IF p_submission_session_id IS NOT NULL THEN
    ...
    IF v_idempotent_id IS NOT NULL THEN
      -- Deterministic order: the per-submission ordinal (input order),
      -- with created_at,id as a stable tiebreaker for legacy NULL rows.
      SELECT COALESCE(
  ...
  IF jsonb_array_length(COALESCE(p_damage_items,'[]'::jsonb)) > 0 THEN
    -- WITH ORDINALITY preserves the input array order in submission_index so a
    -- later idempotent replay returns ids in the same order the client sent.
    FOR v_item, v_ord IN
