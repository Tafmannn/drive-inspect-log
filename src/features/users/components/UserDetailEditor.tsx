...
import { useUserDetail, useUpdateProfile, useSetUserRole, useActivateUser, useSuspendUser, useReactivateUser, useArchiveDriver, useRestoreDriver } from "@/hooks/useUserManagement";
...
export function UserDetailEditor({ userId, onBack }: UserDetailEditorProps) {
  ...
  const isMutating = updateMutation.isPending || setRoleMutation.isPending || activateMutation.isPending || suspendMutation.isPending || reactivateMutation.isPending || archiveMutation.isPending || restoreMutation.isPending;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Role Section */}
      <section className="rounded-lg border bg-card p-4 space-y-3">
        <h3 className="text-xs font-semibold text-foreground">Role & Permissions</h3>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={isMutating || selectedRole === user.role || !canEdit}
          >
            Apply Role
          </Button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {user.account_status === "pending_activation" && (
            <ConfirmAction
              ...
              trigger={
                <Button size="sm" className="h-7 text-xs" disabled={isMutating}>
          ...
          {user.account_status === "active" && !isProtected && (
            <ConfirmActionWithInput
              ...
              trigger={
                <Button size="sm" variant="destructive" className="h-7 text-xs" disabled={isMutating}>
                  <ShieldOff className="h-3 w-3 mr-1" /> Suspend
                </Button>
              }
              onConfirm={() => {
                suspendMutation.mutate({ userId, reason: suspendReason || undefined });
                setSuspendReason("");
              }}
            />
          )}
      ...
      {user.role === "driver" && dp && (
        <section className="rounded-lg border bg-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div><span className="text-muted-foreground">Full Name:</span> {dp.full_name || "—"}</div>
            <div><span className="text-muted-foreground">Active:</span> {dp.is_active ? "Yes" : "No"}</div>
            <div><span className="text-muted-foreground">Licence Expiry:</span> {dp.licence_expiry || "—"}</div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            {!isArchived ? (
              <ConfirmActionWithInput
                ...
                trigger={
                  <Button size="sm" variant="outline" className="h-7 text-xs text-destructive" disabled={isMutating}>
                    <Archive className="h-3 w-3 mr-1" /> Archive Driver
                  </Button>
                }
                onConfirm={() => {
                  archiveMutation.mutate({ userId, reason: archiveReason || undefined });
                  setArchiveReason("");
                }}
              />
            ) : (
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-1 text-[10px]">
                    <input
                      type="checkbox"
                      checked={restoreReactivate}
                      onChange={(e) => setRestoreReactivate(e.target.checked)}
                      className="h-3 w-3"
                    />
                ...
                <ConfirmActionWithInput
                  ...
                  trigger={
                    <Button size="sm" variant="outline" className="h-7 text-xs" disabled={isMutating}>
                      <RotateCcw className="h-3 w-3 mr-1" /> Restore Driver
                    </Button>
                  }
                  onConfirm={() => {
                    restoreMutation.mutate({ userId, reactivate: restoreReactivate, note: restoreNote || undefined });
                    setRestoreNote("");
                  }}
                />
              </div>
