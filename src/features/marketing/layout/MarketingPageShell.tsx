...
interface MarketingPageShellProps {
  ...
  actions?: ReactNode;
  /** Optional photographic hero shown alongside the title. */
  image?: { src: string; alt: string };
}

/**
 * Navy hero band for interior marketing pages — sits directly beneath the solid
 * header and carries the page's single <h1>, optional breadcrumbs/CTAs and an
...
export function MarketingPageShell({
  ...
  return (
    <section className="relative overflow-hidden bg-marketing-hero">
      ...
      <div
        className={cn(
          "relative mx-auto max-w-content px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-16 lg:px-8",
          image && "lg:grid lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-12",
        )}
      >
        <div>
          {breadcrumb && (
        ...
        {image && (
          <div className="mt-10 lg:mt-0">
            <Photo src={image.src} alt={image.alt} ratio="3/2" rounded="3xl" priority />
          </div>
        )}
      </div>
    </section>
