...
export function generatePodEmailBody(
  ...
  lines.push("VEHICLE");
  lines.push(`Registration: ${job.vehicle_reg} – ${job.vehicle_make} ${job.vehicle_model} (${job.vehicle_colour})`);
  if (job.vehicle_year) lines.push(`Year: ${job.vehicle_year}`);
  lines.push(`Job Reference: ${ref}`);
  lines.push("");

  lines.push("JOURNEY OVERVIEW");
  lines.push(`Route: ${job.pickup_city || "Unknown"} → ${job.delivery_city || "Unknown"}`);
  ...
  lines.push(`Contact: ${job.pickup_contact_name} (${job.pickup_contact_phone})`);
  lines.push(`Location: ${job.pickup_address_line1}, ${job.pickup_city}, ${job.pickup_postcode}`);
  if (job.pickup_company) lines.push(`Company: ${job.pickup_company}`);
  lines.push(pickup ? `Date/Time: ${safeDate(pickup.inspected_at)}` : "Date/Time: Not completed");
  lines.push(`Odometer: ${pickup?.odometer != null ? pickup.odometer.toLocaleString("en-GB") : "N/A"}`);
  lines.push(`Fuel: ${fuelLabel(pickup?.fuel_level_percent ?? null)}`);
  if (pickup?.vehicle_condition) lines.push(`Vehicle condition: ${pickup.vehicle_condition}`);
  if (pickup?.light_condition) lines.push(`Light conditions: ${pickup.light_condition}`);
  lines.push(`Recorded damages: ${pickupDamages.length}`);
  lines.push("");

  lines.push("DELIVERY DETAILS");
  lines.push(`Contact: ${job.delivery_contact_name} (${job.delivery_contact_phone})`);
  lines.push(`Location: ${job.delivery_address_line1}, ${job.delivery_city}, ${job.delivery_postcode}`);
  if (job.delivery_company) lines.push(`Company: ${job.delivery_company}`);
  lines.push(delivery ? `Date/Time: ${safeDate(delivery.inspected_at)}` : "Date/Time: Not completed");
  lines.push(`Odometer: ${delivery?.odometer != null ? delivery.odometer.toLocaleString("en-GB") : "N/A"}`);
  ...
  lines.push(`Delivery – Driver: ${deliveryDriver} | Customer: ${deliveryCustomerName || "N/A"}`);
  lines.push("");

  lines.push("DECLARATION");
  lines.push("The customer confirms that the above vehicle has been inspected at the point of delivery and any noted damage or exceptions have been recorded on this POD and accompanying imagery.");
  lines.push("");
